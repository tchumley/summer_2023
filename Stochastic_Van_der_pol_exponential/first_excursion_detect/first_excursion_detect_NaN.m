...
for j = 1:length(mean_interkick_time)
    for k = 1:number_of_trials   
        [tall,Yall,excursion_indicator] = trial(max_time, ... 
            min_kick_size, max_kick_size, lambda, f, IC);
        if excursion_indicator == 1
            t=tall;
            x=Yall(:,1);
...
save(strcat("first_excursion_time", datestr(datetime), ".mat"),"first_excursion_time","max_time","mean_interkick_time")
end
