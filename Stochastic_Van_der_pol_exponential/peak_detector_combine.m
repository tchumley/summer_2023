...
IC = [0.2;-0.288];
max_time = 1000;
mean_interkick_time = 4;
lambda = 1/mean_interkick_time;
number_of_excursion = 0;
number_of_rows =length(maxtab);
inter_peak_time = zeros(number_of_rows,1);
inter_excursion_time = zeros(number_of_rows,1);
filled = 0;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
inter_peak_time(1) = maxtab(1,1);
inter_excursion_time(1) = maxtab(1,1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

for j = 1:length(maxtab)-1
    inter_peak_time(j+1) = maxtab(j+1,1)-maxtab(j,1);
end

% for i = 2:length(inter_excursion_time)
%     if inter_peak_time(i) <= 65
%         while inter_peak_time(i) <= 65
%             inter_excursion_time(i) = inter_excursion_time(i) + inter_peak_time(j) + inter_peak_time(j+1);
%         end
%     else
%         inter_excursion_time(i) = inter_peak_time(i);
%     end
% end
...
save(strcat("inter_excursion_time", datestr(datetime), ".mat"),"number_of_excursion","inter_peak_time", "inter_excursion_time")
