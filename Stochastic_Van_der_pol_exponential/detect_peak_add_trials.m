...
excursion_index = 1;
list_index = 1;
inter_peak_time = zeros(1200,1);
inter_excursion_time = zeros(1200,1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for k = 1:number_of_trials   
    ...
    if list_index == 1
        ...
        for j = 1:length(maxtab)-1
            inter_peak_time(list_index+1) = maxtab(j+1,1)-mintab(j,1);
            list_index = list_index + 1;
        end
    else
        for j = 1:length(maxtab)-1
...
for j = 1:length(inter_peak_time)    
    if inter_peak_time(j) <= threshold
        while (inter_peak_time(j) <= threshold) & (excursion_index <= length(inter_excursion_time))
            ...
            if j+1 <= length(inter_excursion_time)
                j=j+1;
            else
                break
            end
