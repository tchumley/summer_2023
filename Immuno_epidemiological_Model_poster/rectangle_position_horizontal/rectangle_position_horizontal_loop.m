...
for j = 1:length(taus)
    ...
    parfor k = 1:number_of_trials %number_of_trials
        ...
        [excursion_indicator(k),excursion_time(k)] = trial(taus(j));
    end
    ...
    lower_bound = 9000; %kick size

 
    save(strcat("different_rectangle_position_uniform", datestr(datetime), ".mat"), "lower_bound", "which_tau", "excursion_indicator", "excursion_time","mean_time_to_excursion", "probability_of_excursion")
%     temp = temp + 1;
end
toc;
%% 
...
function [excursion_indicator,excursion_time] = trial(which_tau)
...
tau = which_tau+1*rand(floor(600/which_tau),1);
