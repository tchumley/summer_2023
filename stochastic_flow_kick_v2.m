...
for j = 1:length(mean_interkick_time)
    parfor k = 1:number_of_trials
        [excursion_indicator(k), excursion_time(k)] = trial(exp_uniform, ...
            min_kick_size, max_kick_size, lambda(j), ...
            min_interkick_time, max_interkick_time);
    end
    which_lambda = lambda(j);
    mean_time_to_excursion = mean(excursion_time(~isnan(excursion_time)));

    save(strcat("excursion_result_time_poisson_different_lambda", datestr(datetime), ".mat"), "excursion_indicator", "excursion_time", "which_lambda",'mean_time_to_excursion')
    histogram(excursion_time(~isnan(excursion_time)), 9, 'Normalization','PDF')
    savefig(strcat("histogram", datestr(datetime), "mean_interkick_time", num2str(1/which_lambda), ".fig"))
end
toc;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%% Save data %%%%%%%%%%%%%%%%%%%%%%
if exp_uniform == 1
    save(strcat("excursion_poisson", datestr(datetime), ...
